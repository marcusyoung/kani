...
- [ ] Ensure fallback attempts restyle the payload for each fallback provider instead of reusing a provider-specific payload from the primary attempt. (verification: integration - fallback test with primary `openai` and fallback `dashscope` confirms fallback body uses `enable_thinking` and not stale `reasoning`)
- [ ] Update sample configuration or README configuration text to document `reasoning_style` values and default behavior. (verification: manual - documentation mentions supported values and default `openai`)
- [ ] Run repository quality checks for changed Python behavior. (verification: integration - `uv run ruff check src/`, `uv run ruff format --check src/ tests/`, `uv run pyright src/`, and targeted pytest pass)
