...
#### Scenario: モデルエントリの provider が tier provider より優先される

- GIVEN tier に `provider` が設定されている
- AND 選択された primary または fallback のモデルエントリにも `provider` が設定されている
- WHEN ルーティングで provider を解決する
- THEN モデルエントリの `provider` を使用する
- AND tier の `provider` はその候補には適用されない

#### Scenario: configured model IDs are literal

- GIVEN モデルエントリの `model` が `provider/model-name` のような slash を含む文字列である
- WHEN ルーティングが upstream request を構築する
- THEN システムは設定された `model` 文字列を selected provider に literal model ID として送信する
- AND slash 区切りを kani provider selector として解析してはならない
- AND provider selection は model entry provider, tier provider, `default_provider` の優先順位のみで決定される
